...
계획서: `.omc/plans/ux-reclassification-cn-redesign.md` (v2)

#### Feature 3: 토지이용 재분류 UX 개선 (Tab 1 Split View)
- [x] Task 1: 임포트 추가 (QComboBox, QSplitter) + `_get_cn_land_use_names()` 유틸리티
- [x] Task 2: `_MappingComboDelegate` 클래스 + `setItemDelegateForColumn()` 분리 적용
- [x] Task 3: `_validate_mapping_cell()` — 재분류명 유효성 배경색 표시
- [x] Task 4: Tab 1 QSplitter 레이아웃 — 좌: 매핑 테이블, 우: CN표 참조 패널
- [x] Task 5: `_load_cn_ref_table()`, `_filter_cn_ref_table()`, `_sync_cn_ref_from_edit()` + dirty 플래그
- [x] Task 6: 시그널 연결 + `_on_tab_changed()` TAB_MAPPING 분기 확장
- [x] Task 7: `_on_cn_ref_double_click()` — 더블클릭 → 매핑 자동 입력
- [x] Task 8: 안내 문구 업데이트
