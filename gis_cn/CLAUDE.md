...
DB에서 Clip+Intersection을 단일 CTE 쿼리로 처리 → QGIS Processing 최소화.
- `get_soil_layer`/`get_land_cover_layer`: ST_Intersection으로 clip 결과 직접 반환
- `get_soil_lc_intersection`: CTE 4단계(토양clip→토지clip→Dissolve→Intersection) 단일 쿼리
