...
## ⚠️ 긴급 해결 과제 (2026-06-30) — 입력 좌표계(.prj) 미검증으로 토양군 빈 결과

**증상:** 특정 유역 SHP로 실행 시 `토양군_clip`에 대면적 빈 구멍 발생 → CN 산정이 빈값/실패로 들어옴. (사용자 제보 + 스크린샷 IMG_8774)

**근본 원인(규명 완료):** 입력 SHP에 **`.prj` 없음 → 좌표계 미정의**. 실제 좌표계는 **중부원점 false-northing 50만(EPSG:5174/5181)** 인데 **5186(FN 60만)으로 잘못 인식**되어 유역이 정북으로 약 100km 어긋남(전북→전남). 어긋난 위치는 토양군 갭이 많아 절반 이상이 비었던 것.
- 진단 근거(동일 유역, CRS만 변경): **5186 → 토양군 덮음 42.7%**(전남 나주) vs **5174/5181 → 토양군 100%**(전북). land_cover는 양쪽 모두 100%.
- 결론: **토양군 DB도, 플러그인 계산도 정상.** 원인은 **입력 CRS 오정렬**. 다만 플러그인이 이 흔한 한국 CRS 함정을 못 막은 책임 있음.

**해결할 일(플러그인 가드 — 우선순위):**
1. **[필수] 입력 CRS 검증**: 입력 레이어 CRS가 미정의/지리좌표(경위도)/비-한국TM이면 실행 전 **경고 + 좌표계 지정 유도**. `.prj` 없는 SHP는 그대로 진행 금지.
2. **[필수] 커버리지 자동 경고**: clip 후 `land_cover 덮음 ≈ 100%인데 토양군 덮음 < 임계(예: 90%)`면 **"좌표계 의심 — 토양군 X% 누락"** 경고. (이번 케이스 land 100% / soil 42.7% = 명백한 오정렬 신호)
3. **[권장] 소유역명 필드 검증**: CAD(DXF) 변환 SHP(ENTITY/LAYER/COLOR 등)는 소유역명 필드 부재 → 안내.
4. **[선택] 토양군 갭 처리 옵션**: 최근접 토양군 nearest-fill 또는 면적 임계 미만 무시.

**사용자 임시 회피:** 유역 레이어 CRS를 **EPSG:5174(또는 5181)** 로 "지정"(재투영 아님) 후 재실행 → 정위치(전북) clip, 토양군 100%.
